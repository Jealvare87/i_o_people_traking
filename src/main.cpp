 ...
 * 
**/
Mat detectVideo(Mat frame, tResult &m, CentroidTracker *centroidTracker, unique_ptr<tflite::Interpreter> &interpreter, vector<string> labels)
{
    Mat image;
    TFlite_C tfl;
    tfl.sLabels(labels);
    ...
    // Set visual limit
    line(frame, Point(0, cam_height / 2 - 25), Point(cam_width, cam_height / 2 - 25), Scalar(103, 255, 51), 2);
    line(frame, Point(0, cam_height / 2), Point(cam_width, cam_height / 2), Scalar(0, 0, 255), 2);
    line(frame, Point(0, cam_height / 2 + 25), Point(cam_width, cam_height / 2 + 25), Scalar(103, 255, 51), 2);
    ...
    if (!objects.empty())
    {
        for (auto obj : objects)
        {
            ...
            putText(frame, ID, Point(obj.second.first - 10, obj.second.second - 10),
                    FONT_HERSHEY_COMPLEX, 0.5,Scalar(0, 152, 255), 2);

            // Limits
            int med = cam_height / 2;
            int up = med - 25;
            int down = med + 25;
            ...
            for (int i = 1; i < centroidTracker->path_keeper[obj.first].size(); i++)
            {
                ...
                /*
                    Enter-Exit Counter
                */
                if ((centroidTracker->path_keeper[obj.first][i - 1].second < med && centroidTracker->path_keeper[obj.first][i - 1].second > up) &&
                    (centroidTracker->path_keeper[obj.first][i].second > med && centroidTracker->path_keeper[obj.first][i].second < down))
                {
                    m._entry++;
                    aux = centroidTracker->path_keeper[obj.first][i];
                    centroidTracker->path_keeper[obj.first].clear();
                    centroidTracker->path_keeper[obj.first].push_back(aux);
                }
                else if ((centroidTracker->path_keeper[obj.first][i - 1].second > med && centroidTracker->path_keeper[obj.first][i - 1].second < down) &&
                         (centroidTracker->path_keeper[obj.first][i].second < med && centroidTracker->path_keeper[obj.first][i].second > up))
                {
                    m._exit++;
                    aux = centroidTracker->path_keeper[obj.first][i];
                    centroidTracker->path_keeper[obj.first].clear();
...
int main(int argc, const char **argv)
{
    ...
    initPublisher();
    VideoCapture vs;
    auto centroidTracker = new CentroidTracker(70);
    ...
    while (1)
    {
        ...
        rez = detectVideo(frame, m, centroidTracker, interpreter, labels);

        // When the value is modified, an event is created 
        if (m._old_en != m._entry || m._old_ex != m._exit)
        {
            publicar(m._entry, m._exit, "zona1/raspi1");
            m._old_en = m._old_en != m._entry ? m._entry : m._old_en;
            m._old_ex = m._old_ex != m._exit ? m._exit : m._old_ex;
        }
        ...
        putText(rez, format("FRAME %0.2d", totalFrames), Point(10, 20), FONT_HERSHEY_SIMPLEX, 0.6, Scalar(0, 245, 255), 2);

        ++totalFrames;

        imshow("Occupancy Control", rez);
